...
app.use((req, res, next) => {
  ...
  setTimeout(() => {
    if (!didRespond) {
      console.log(chalk.bold.yellow(`No response received within ${responseTimeout} ms. Sending HTML code...`));
      res.status(500).sendFile(path.join(__dirname, "public", "timeout.html"));
    }
  }, responseTimeout);
...
function startServer() {
  try {
    ...
    app.use((req, res, next) => { res.status(404).sendFile(path.join(__dirname, "public", "notf.html"));
});
